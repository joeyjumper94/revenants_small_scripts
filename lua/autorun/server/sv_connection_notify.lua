...
hook.Add("player_connect","sv_connection_notify",function(data)
	...
	for k,v in ipairs(player.GetAll()) do
		if v and v:IsValid() and v:IsSuperAdmin() then
...
hook.Add("player_disconnect","sv_connection_notify",function(data)
	local name	 =data.name		--Same as Player:Nick()
	local steamid=data.networkid--Same as Player:SteamID()
	local id	 =data.userid	--Same as Player:UserID()
	local bot	 =data.bot		--Same as Player:IsBot()
